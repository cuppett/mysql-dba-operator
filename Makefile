...
# Add a new line here if you would like to change its default config. (E.g DEFAU)LT_CHANNEL = "stable")
...
IMAGE_TAG_BASE ?= quay.io/cuppett/mysql-dba-operator
...
POD_NAMESPACE ?= mysql-dba-operator-system
...
go mod init tmp ;\
echo "Downloading $(2)" ;\
GOBIN=$(PROJECT_DIR)/bin GO111MODULE=on go get $(2) ;\
rm -rf $$TMP_DIR ;\
}
endef
